...
export function RecentApplications() {
  ...
  const loadApplications = async () => {
    try {
      ...
      // Filter pending applications and sort by date, then take the 2 most recent
      const recentPendingApplications = data
        .filter(app => app.status === "pending")
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
        .slice(0, 2)
      setApplications(recentPendingApplications)
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleStatusUpdate = async (applicationId: string, status: "approved" | "rejected") => {
    try {
      ...
      loadApplications() // Reload the list to show updated recent pending applications
    } catch (error) {
      toast({
        title: "Error",
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold">Recent Pending Applications</h2>
